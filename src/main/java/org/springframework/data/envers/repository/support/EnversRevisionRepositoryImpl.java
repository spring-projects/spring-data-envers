...
public class EnversRevisionRepositoryImpl<T, ID extends Serializable, N extends Number & Comparable<N>> extends
    ...
    public Page<Revision<N, T>> findRevisions(ID id, Pageable pageable) {
        ...
        Revisions<N, T> revisions = getEntitiesForRevisions(
                addSort(auditQuery, pageable).getResultList(), id, reader);
                        ...
                        case DYNAMIC_PROPERTY:
                            ...
                            if (!StringUtils.isEmpty(prop)) {
                                if (StringUtils.startsWithIgnoreCase(prop, REVISION_PROPERTY_PREFIX)) {
                                    auditProperty = AuditEntity.revisionProperty(
                                            prop.substring(REVISION_PROPERTY_PREFIX.length()));
                                } else if (StringUtils.startsWithIgnoreCase(prop, AUDITED_ENTITY_PROPERTY_PREFIX)) {
                                    auditProperty =
                                            AuditEntity.property(prop.substring(AUDITED_ENTITY_PROPERTY_PREFIX.length()));
                                }
                            }
                            break;
                    }
